...
public interface UserRepository extends JpaRepository<User, Integer> {
	...
	public List<User> findUserByDepartment(String name,Integer departmentId);
	
	@Query(value="select * from user where name=?1 and department_id=?2",nativeQuery=true)
	public List<User> nativeQuery(String name,Integer departmentId);
	
	@Query(value="select * from user where name=:name and department_id=:departmentId",nativeQuery=true)
	public List<User> nativeQuery2(@Param("name")  String name,@Param("departmentId")  Integer departmentId);
	...
	@Query(value="select u from User u where u.departmentId=?1")
	public Page<User> queryUsers(Integer departmentId,Pageable page);
	
	@Transactional
    @Modifying
    @Query(value = "delete from User u where u.name =?1 and u.pwd=?2")
    void delUser(String name,String pwd);
